...
#         After location (where it is), you answer about formation (DJ, band, ensemble, or solo artist?). After formation you answer about genre type (Interesting choice! Which music genre are you interested in?). After genre type you answer about price (how much a customer is willing to pay). Provide each new answer with different topic SEPARATELY after receiving each user input and DO NOT generate Raw code. Finally, after receiving user input say something like "Noted. I'll show you some artists that might interest you."
#         {context}
...
#     res = await chain.acall({"question": message.content, "chat_history": message.content}, callbacks=[cb])
#     answer = res["answer"]
